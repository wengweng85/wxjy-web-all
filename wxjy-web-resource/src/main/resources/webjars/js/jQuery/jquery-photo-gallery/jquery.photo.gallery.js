...
$.fn.extend({
	...
	photoGallery: function(options) {
 	 	 	...
 	 	 	template : {
	 	 	 	...
	 	 	 	OPERTATION : '<div class="oper">' +
							 ...
							 '<div id="wordTool" class="tool">' +
								'<div class="toolct">' +
									'<span class="oper_word" title="在线预览"><i class="icon_tool-openfile"></i></span>' +
									'<span class="oper_download" title="下载"><i class="icon_tool-download"></i></span>' +
								'</div>' +
							 '</div>' +
							 '<div id="excelTool" class="tool">' +
								'<div class="toolct">' +
									'<span class="oper_excel" title="在线预览"><i class="icon_tool-openfile"></i></span>' +
									'<span class="oper_download" title="下载"><i class="icon_tool-download"></i></span>' +
								'</div>' +
							 '</div>' +
							 '<div id="otherTool" class="tool">' +
								'<div class="toolct">' +
									'<span class="oper_download" title="下载"><i class="icon_tool-download"></i></span>' +
								'</div>' +
							 '</div>',
		...
		var o = $.extend(defaults, options),
      	...
      	var $imgTool = $(this).find("#imgTool"),
            $wordTool = $(this).find("#wordTool"),
            $excelTool = $(this).find("#excelTool"),
            $otherTool = $(this).find("#otherTool"),
			...
			$prev = $(this).find(".prev"),
			$next = $(this).find(".next"),
            $openword = $(this).find(".oper_word"),
            $openexcel = $(this).find(".oper_excel"),
	 	...
	 	$thumbnails.find(".thumbClose").on("click",function(){
      	...
      	//显示工具栏
  	  	$gallery.on("mouseover",function(e){
  	  		var dataType = $image.attr('data-type');
  	  		if(dataType == 'image'){
  	  			$imgTool.show();
		...
		var isMax,preWidth, preHeight, preTop, preLeft;
  	 	$fullscreen.on("click", function(){
			...
			if(!isMax){
                if(parent.$('.navbar-fixed-top')){
                    difHeight = parent.$('.navbar-fixed-top').height();
                }else{
                    difHeight = 0;
                }
                var maxHeight1 = document.body.clientHeight - windowMargin * 2 - difHeight;
				isMax = true;
				...
				J.css({
					...
					height : parentD.body.clientHeight - difHeight,
				});
			} else{
				isMax = false;
				J.css({
        ...
        $openword.on("click", function(){
            var file_uuid = $image.attr("data-id");
            alert('暂无在线预览功能');
        });

        //在线打开
        $openexcel.on("click", function(){
            var file_uuid = $image.attr("data-id");
            alert('暂无在线预览功能');
        });
	  	...
	  	function init(){
  	    	...
  	    	$(o.imgs).each(function(i, img){
	  	    	$(o.template.IMAGE)
	  	    		.appendTo($gallery)
	  	    		.attr("src", img.url)
                    .attr("data-type", img.dataType)
                    .attr("data-id", img.dataId)
	  	    		.attr("index", i)
	  	    		.css({
				  	 	width : img.imgWidth,
...
$.extend({
	...
	openPhotoGallery : function(obj){
		...
		if(!imgUrl) return;
		if($('.navbar-fixed-top')){
            difHeight = $('.navbar-fixed-top').height();
		}else{
            difHeight = 0;
		}
		...
		var img = $img[0],
			...
			winHeight,
			winWidth,
			maxHeight = document.body.clientHeight - windowMargin * 2 - difHeight,
		...
		$("<iframe></iframe").appendTo("body")
			.attr("id", "J_pg")
			.attr("src", contextPath+"/common/gallery")
			.css({
				position : "absolute",
				left : (document.body.clientWidth - winWidth) /2,
				top : (document.body.clientHeight + difHeight - winHeight) /2 + $(document).scrollTop(),
				width : winWidth,
				height : winHeight,
