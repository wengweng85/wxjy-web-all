...
var rc_tag = {
	...
	dynamic_get_codevalue_by_codetype_and_filter:function(current_selector,codetype,filter,value){
		if(codetype&&filter){
			var url=gateway_base_url+'/api-code/codetype/getInitCodeValueList';
			rc.api_post(url,{code_type:codetype,filter:filter},function(response){
				//将当前节点后的所有子选择框清空
	...
	dynamic_get_codevalue_by_codetype:function(current_selector,codetype,value){
		if(codetype){
			var url=gateway_base_url+'/api-code/codetype/getInitCodeValueList';
			rc.api_post(url,{code_type:codetype},function(response){
				//将当前节点后的所有子选择框清空
				$(current_selector).empty();
				//$(current_selector).selectpicker('refresh');
				//数据加模型生成新的option数组
	...
	code_value_select_data_change:function(currentvalue,next_code_type,next_selector){
		var url=gateway_base_url+'/api-code/codetype/queryCodeValueByCodeTypeAndParent';
		rc.api_post(url,{par_code_value:currentvalue,code_type:next_code_type},function(response){
			//将当前节点后的所有子选择框清空
