...
<form class="container" id="form" action="<%=path%>/catalogue/perServiceHall" method="post">
  <div class="row">
    <div class="col-xs-6">
      <ol class="breadcrumb">
        <li class="active">个人服务大厅</li>
      </ol>
    </div>
  ...
  <c:forEach items="${busTypeList}" var="bus">
    <div class="bg-white text-primary f18 mt-10" style="padding: 20px 30px;border-bottom: 2px solid #e1e0e0">
      <img src="<%=fileModule%>${bus.bus_type_bigimg}" alt="" width="30px">
      <strong>${bus.bus_type_name}</strong>
    </div>
    <ul class="v-cata-list">
      <c:forEach items="${listMap[bus.bus_type_id]}" var="l">
        <li>
          ...
          <div class="pull-right">
            ...
            <button type="button" class="btn btn-primary-plain" onclick="toggleCollect(this, '${l.cata_id}')">
              <c:choose>
                <c:when test="${l.is_collect == '1'}">
                  取消收藏
                </c:when>
                <c:otherwise>
                  收藏
