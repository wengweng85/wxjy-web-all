    ...
    public void contextInitialized (ServletContextEvent arg0) {
        ...
        if (redisswitch.equals("on")) {
            ...
            try {
               //需要同步
               if (syn_flag) {
                 list_code_type = new HttpRequestUtils<CodeType>(gateway_base_url,isencrpty,contextype).httpPostReturnList(ApiUriContraints.API_CODE_CODETYPELIST, CodeType.class);
                 ...
                 fixedThreadPool.execute(new Runnable() {
                       public void run() {
                              ...
                              try{
                                  ...
                                  List<CodeValue> list_code_value = new HttpRequestUtils<CodeValue>(gateway_base_url,isencrpty,contextype).httpPostReturnList(ApiUriContraints.API_CODE_CODEVALUElIST, map,CodeValue.class);
        ...
        //是否同步标志 如果上一次同步时间是1小时之内，不同步下载代码
        try {
            List<CodeType> list_code_type = new HttpRequestUtils(gateway_base_url,isencrpty,contextype).httpPostReturnList(ApiUriContraints.API_CODE_CODETYPELIST, CodeType.class);
            ExecutorService fixedThreadPool = Executors.newFixedThreadPool(10);
            for(int i = 0; i < list_code_type.size(); i++){
                 CodeType codeType = list_code_type.get(i);
                 final String code_type = codeType.getCode_type();
                 fixedThreadPool.execute(new Runnable() {
                    public void run() {
                        ...
                        try{
                            HashMap map=new HashMap();
                            map.put("code_type", code_type);
                             List<CodeValue> list_code_value = new HttpRequestUtils(gateway_base_url,isencrpty,contextype).httpPostReturnList(ApiUriContraints.API_CODE_CODEVALUElIST, map, CodeValue.class);
                             if (list_code_value.size() > 0) {
                                 //将代码参加加载到ehcache缓存中
